...
function generatedPassword(upper, lower, number, symbol, length) {
  ...
  // The filter method creates a new array with all the elements that pass teh test implemented by the provided function
  ...
  // Removing extra characters if necessary (The Above loop witll create a password that may not match the length selected if that length is not a multiple fo the number of the types selected)
...
// generatedPassword(true, false, false, false, 5);
...
generatEl.addEventListener(`click`, () => {
  ...
  // Chekcing if the following options/checkedboxes are selected/checked and setting the returned true/false values to the respective variables
...
// The generatePassword function takes the true/false values determined by the checkboxes as well as the number from the number input as arguments and returns a sting (AKA The generated Password) which set as the value for the innerText property for the result element/span
...
// If user click clipboard while no password is displayed the function will end and nothing will be copied to clipboard
if (password === ""){
  alert( `Please generate a password first`);
  return;
}

// Setting the value fro the textArea to the password tha is currently being displayed
